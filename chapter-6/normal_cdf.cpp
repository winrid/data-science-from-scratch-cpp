...
int main() {
    ...
    for (int i = -50; i <= 50; i++) {
        int x = i / 10;
        xs.push_back(x);
        sigma_1.push_back(probabilities::normal_cdf(x, 0, 1));
        sigma_2.push_back(probabilities::normal_cdf(x, 0, 2));
        sigma_0_5.push_back(probabilities::normal_cdf(x, 0, 0.5));
        sigma_1_mu_ne1.push_back(probabilities::normal_cdf(x, -1, 1));
    }
